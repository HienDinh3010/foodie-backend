...
export const searchRecipe = async (req: Request, res: Response) => {
  ...
  try {
    const pipeline = [
      {
        $search: {
          ...
          text: {
            ...
            fuzzy: {},// allow to search for a wrong keywork such as "chickn" => "chicken"
          },
        },
      },
      {
      ...
      {
        $project: {// what kind of data we want to get
          user: 1,
          note: 1,
          description: 1,
          title: 1,
    ...
    if (!!recipes?.length) {//if having at least one recipe
...
export const createRecipe = async (req: Request, res: Response) => {
  if (!req?.user) {// If don't have a user yet
